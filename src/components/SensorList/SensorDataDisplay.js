...
function TemperatureDisplay({ temperature }) {
  return <span>{temperature} °C</span>
}
      ...
      <div className='displayContainer'>
        ...
        {sensorData.length > 0 ? (
          <ul>
            {sensorData.map((data) => (
              <li key={data._id}>
                ID Датчика: {data.sensor_id}, Температура: <TemperatureDisplay temperature={data.temperature} />, Дата
                замера: {new Date(data.timestamp).toLocaleString()}, Влажность: {data.humidity} %, Культура:{' '}
                {data.crop_type}
              </li>
            ))}
          </ul>
